...
import { follow, mouseon, unFollow } from "../../utils/utils";
import {
  collection,
  onSnapshot,
  orderBy,
...
const Wrapper = styled(motion.div)`
  ...
  ${({ $position, $top }) =>
    $position
      ? `left: ${$position[0]}px; top: ${
          $position[1] + ($top ? Number($top) : 22)
        }px`
      : "left:0px; top:0px;"}
`;
...
const HoverProfile = ({ type, uid, top, position }) => {
  ...
  const followAction = async () => {
    if (!followResult) {
      follow(myProfile.uid, uid);
    } else {
      unFollow(myProfile.uid, uid);
    }
  };

  const moveProfile = () => {
    if (!hoverProfile) return;
    navigate({
      pathname: "/profile",
      search: `?${createSearchParams({
        userId: hoverProfile.userId,
      })}`,
    });
  };
        ...
        <PostingPics>
          {hoverFeed &&
            hoverFeed.map((it, idx) => (
              <ImgBox key={idx} onClick={() => moveProfile()}>
