...
const updateFollowing = async (searchUid, filedName, insertUid) => {
  ...
  if (!querySnapshot.empty) {
    ...
    await updateDoc(docRef, { [filedName]: arrayUnion(insertUid) });
  }
};

export const follow = async (myUid, userUid) => {
  await Promise.all([
    updateFollowing(myUid, "following", userUid),
    updateFollowing(userUid, "follower", myUid),
  ]);
};

const updateUnFollowing = async (searchUid, filedName, insertUid) => {
  const searchQuery = query(
    collection(db, "profile"),
    where("uid", "==", searchUid)
  );
  ...
  if (!querySnapshot.empty) {
    ...
    await updateDoc(docRef, { [filedName]: arrayRemove(insertUid) });
  }
};

export const unFollow = async (myUid, userUid) => {
  await Promise.all([
    updateUnFollowing(myUid, "following", userUid),
    updateUnFollowing(userUid, "follower", myUid),
  ]);
};
